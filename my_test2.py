...
def simulate_2(n_ch = 2):
    ...
    sig_np, source_path = get_sources(n_ch)
...
for i in range(20):
    ...
    mixed_wav, ori_sig = simulate_2(n_ch)
    ...
    algorithm_list = ['my_ilrma_t','wpe','wpe+ilrma-IP','ilrma-t-IP', 'ilrma-t-iss-seq', 'ilrma-t-iss-joint', 'ilrma-IP', 'ilrma-iss' ,'auxiva']
    ######################################################################################################################
    ...
    ##### 评估
        ...
        file_txt = open('test_wav/mytest__'+str(n_ch)+'.txt', mode='a')
        file_txt.write(algorithm+'\t'+str(ds.item())+'\t'+str(np.mean(dsd))+'\t'+str(np.mean(dd))+'\n')
        file_txt.close()
