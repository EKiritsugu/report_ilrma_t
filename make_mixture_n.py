...
def get_mixed_sig(n_sources = 2, SNR = None):
    ...
    def add_noise(mixed_sig ):
        ...
        var = np.sqrt(n_sources * 10**(-SNR/10) )
...
def update_excel():
    ...
    df.loc[row] = [n_s ,src_path, si_sdr0 , si_sir0 , si_sar0, si_perm, si0,  sdr0 , sir0 , sar0, perm]
    pd.DataFrame(df).to_excel(excel_path, index=False)


n_samples = 333
n_ch = 2
path_save = 'mixed_wn/' + str(n_ch) + 'ch/'
excel_path = 'mixed_wn/' + 'ch_'+str(n_ch)+'.xlsx'
ini_excel()
print('working')
